...
void realizarRonda(General generales[], int rank, int size) {
    ...
    for (int i = 0; i < NUM_GENERALES; i++) {
        // Inicializa el generador de números aleatorios con el ID del proceso como semilla
...
int elegirRey(General generales[], int rank, int size) {
    ...
    for (int i = 0; i < NUM_GENERALES; i++) {
        // Verifica si el general actual no es un traidor, no ha sido elegido como rey previamente y tiene un ID mayor que el máximo encontrado hasta ahora.
        if (!generales[i].es_traidor && !generales[i].es_traidor && generales[i].id > max_id) {
